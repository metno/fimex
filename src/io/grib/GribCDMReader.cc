...
public:
    GribVarIdx()
        : edition_(0)
        , params_(vector<long>(3, 0))
    {
    }
    GribVarIdx(long edition, vector<long> params)
        : edition_(edition)
        , params_(params)
    {
    }
    bool operator<(GribVarIdx rhs) const
    {
        ...
        if (edition_ < rhs.edition_)
            return true;
        else if (edition_ > rhs.edition_)
            return false;

        // editions equals, test the param-vectors
        return lexicographical_compare(params_.begin(), params_.end(), rhs.params_.begin(), rhs.params_.end());
    }
    bool operator==(GribVarIdx rhs) const
    {
        if (edition_ == rhs.edition_) {
            if ((params_.size() == rhs.params_.size()) && equal(params_.begin(), params_.end(), rhs.params_.begin())) {
                return true;
            }
        }
        return false;
...
// based upon ==
bool operator!=(const GribVarIdx& lhs, const GribVarIdx& rhs)
{
    return !(lhs == rhs);
}
// based upon <
...
void GribCDMReader::initSpecialLevels_(xmlNodePtr node, const string& extension, const string& levelType, size_t levelPos, const vector<string>& levelShape,
                                       DataPtr& levelData)
{
...
void GribCDMReader::initCreateGFIBoxes()
{
    ...
    for (const auto& varTypeIt : varLevelType) {
        ...
        vector<vector<long>>::iterator posIt = find_if(levelForType.begin(), levelForType.end(), EqualFunctor<set<long>, vector<long>>(levels));
        size_t pos;
...
void GribCDMReader::initAddProjection()
{
    ...
    for (map<GridDefinition, string>::iterator gd = gridDefs.begin(); gd != gridDefs.end(); gd++, lastXYId++) {
        ...
        string longName, latName;
        {
            ...
            int found =
                readXPathNodeWithCDMAttributes(*(p_->doc), xpathStringLong, lonlatXmlAttributes, lonlatVarAttributes, p_->templateReplacementAttributes);
            if (found != 1) {
    ...
    for (const GribFileMessage& gfm : p_->indices) {
        ...
        if (initializedVariables.find(varName) == initializedVariables.end()) {
            ...
            if (node != 0) {
                xmlNodePtr varNodeChild = node->children;
                while (varNodeChild != 0) {
                    if ((varNodeChild->type == XML_ELEMENT_NODE) && (string("precision") == reinterpret_cast<const char*>(varNodeChild->name))) {
                        double scale = 1.;
                        double offset = 0.;
