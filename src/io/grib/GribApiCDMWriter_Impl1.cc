        ...
        if (projection == "stereographic" || projection == "polar_stereographic") {
            ...
            if (std::fabs(latitudeWhereDxAndDyAreSpecifiedInDegrees - 60.) > 1.e-5) {
                LOG4FIMEX(logger, Logger::ERROR,
                          "grib1 does not support polar_stereographic with lat_ts != 60degree, got " << latitudeWhereDxAndDyAreSpecifiedInDegrees);
            ...
            if (cdm.getLatitudeLongitude(varName, latitude, longitude)) {
                GRIB_CHECK(
                    grib_set_double(gribHandle.get(), "latitudeOfFirstGridPointInDegrees", cdmReader->getScaledDataInUnit(latitude, "degree")->asDouble()[0]),
                    "");
                GRIB_CHECK(
