...
FeltParameters::FeltParameters(const std::vector<std::string>& dianaFeltParams, const std::string& globalRestrictions)
{
    for (std::vector<std::string>::const_iterator it = dianaFeltParams.begin(); it != dianaFeltParams.end(); ++it) {
        ...
        // cerr << "Parameter " << paramName << "/" << cleanParamName << ": " << parameterDatatypeMap[paramName] << " " << parameterFillValueMap[paramName] <<
        // endl;
    }
}
...
std::array<short, 16> FeltParameters::diana2feltparameters(const std::string& dianaString)
{
    ...
    for (const std::string& ii : MetNoFimex::split_any(dianaStrings.front(), ",")) {
        ...
        switch (i++) {
        case 0:
            diana2feltParameters[11] = value;
            break; // param
        case 1:
            diana2feltParameters[10] = value;
            break; // v.coord
        case 2:
            diana2feltParameters[12] = value;
            break; // level
        }
    }
...
std::string getProjString(int gridType, const std::array<float, 6>& gridParameters)
{
    ...
    case 1:
    case 4:
        tempProj << "+proj=stere +lat_0=90 +lat_ts=" << gridParameters[4] << " +lon_0=" << gridParameters[3] << " " << earth;
        break;
    case 2:
        tempProj << "+proj=latlong " << earth; // geographic
        break;
    case 3:
        tempProj << "+proj=ob_tran +o_proj=latlong +o_lat_p=" << (90 - gridParameters[5]) << " +o_lon_p=0 +lon_0=" << gridParameters[4] << " "
                 << earth; // rotated geographic
        break;
    case 5:
        tempProj << "+proj=gstmerc +lat_1=" << gridParameters[4] << " " << earth; // mercator ???
        break;
    case 6:
        tempProj << "+proj=lcc +lon_0=" << gridParameters[4] << " +lat_0=" << gridParameters[5] << " +lat_1=" << gridParameters[5]
                 << " +lat_2=" << gridParameters[5] << " " << earth;
        break;
    default:
        throw Felt_File_Error("unknown projection-id: " + MetNoFimex::type2string(gridType));
    }
    return tempProj.str();
}
