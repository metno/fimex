...
FeltGridDefinitionPtr FeltField::projectionInformation() const
{
    ...
    if (gType > 999)
        gType /= 1000;
    FeltGridDefinitionPtr ret =
        std::make_shared<FeltGridDefinition>(gType, xNum(), yNum(), static_cast<short int>(getGridHeader_()[14]), static_cast<short int>(getGridHeader_()[15]),
...
std::string FeltField::gridInformation() const
{
    ...
    cont << "\txNum\t\t" << b[9] << "\n";
    cont << "\tyNum\t\t" << b[10] << "\n";
    cont << "\tTime\t\t" << b[11] << '-' << b[12] / 100 << '-' << b[12] % 100 << 'T' << b[13] / 100 << ':' << b[13] % 100 << ":00\n";
