                                   ...
                                   const std::array<float, 6>& gridParameterDelta)
{
    ...
    for (int i = 0; i < 6; i++) {
        ...
        if (newParams[i] != defaultParams[i] && std::fabs(newParams[i] - defaultParams[i]) > gridParameterDelta[i]) {
            throw Felt_File_Error("cannot change gridParameters within a file for " + getName() + " gridParameter (c-counting) " + type2string(i) + ": " +
                                  type2string(defaultParams[i]) + " != " + type2string(newParams[i]) + "(" + type2string(newParams[i] - defaultParams[i]) +
                                  ")");
        }
    }
...
{
    if (feltFields_.size() == 1 && defaultField_->dataType() == 4) {
