...
int mifi_fill2d_f(size_t nx, size_t ny, float* field, float relaxCrit, float corrEff, size_t maxLoop, size_t* nChanged) {
    ...
    *nChanged = 0;
    ...
    if (wField == NULL) {
        fprintf(stderr, "error allocating memory of float(%lu*%lu)", (unsigned long)nx, (unsigned long)ny);
        exit(1);
    }
...
int mifi_creepfill2d_f(size_t nx, size_t ny, float* field, unsigned short repeat, char setWeight, size_t* nChanged) {
    ...
    *nChanged = 0;
    ...
    if (wField == NULL) {
        fprintf(stderr, "error allocating memory of char(%lu*%lu)", (unsigned long)nx, (unsigned long)ny);
        exit(1);
    }
    unsigned short* rField = (unsigned short*) malloc(totalSize*sizeof(unsigned short));
    if (rField == NULL) {
        fprintf(stderr, "error allocating memory of short(%lu*%lu)", (unsigned long)nx, (unsigned long)ny);
        exit(1);
    }
