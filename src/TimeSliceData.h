...
/**
 * Abstract class for a DataSlice of a certain time
 */
class TimeSliceData
{
public:
	TimeSliceData() {}
	virtual ~TimeSliceData() {}
...
template<class C>
class TimeSliceDataImpl : public TimeSliceData
{
public:
	explicit TimeSliceDataImpl(int variableId, Time time, long length, boost::shared_array<C> data)
	: variableId(variableId), time(time), length(length), theData(data) {}
	virtual ~TimeSliceDataImpl() {}

	virtual long size() const {return length;}
	virtual int getVariableId() const {return variableId;}
