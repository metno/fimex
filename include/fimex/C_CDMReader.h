 ...
 */
class C_CDMReader: public MetNoFimex::CDMReader
{
public:
    C_CDMReader();
    virtual ~C_CDMReader();
    virtual boost::shared_ptr<Data> getDataSlice(const std::string& varName, size_t unLimDimPos) throw(CDMException) = 0;
    void setDoubleCallbackFunction(const char* varName, int (callback)(const char* varName, size_t unLimDimPos, double** scaledData, size_t dataSize));
};
